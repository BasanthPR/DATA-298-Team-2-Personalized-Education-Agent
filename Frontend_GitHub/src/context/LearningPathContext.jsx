...
export const LearningPathProvider = ({ children }) => {
    ...
    const createNewPath = async (title, newMilestones, graphData) => {
        ...
        // Optimistic update so dashboard shows content immediately on navigate
        ...
        setLearningPaths(prev => [tempPath, ...prev]);
        setCurrentPathId(tempId);
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Replace the temp entry with the real server-assigned path
                setLearningPaths(prev => prev.map(p => p.id === tempId ? newPath : p));
    ...
    const updateMilestone = async (id, updatedData) => {
        ...
        const token = localStorage.getItem('auth_token');
        if (token) {
            await fetch(`${API_BASE}/api/paths/milestone/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const deletePath = async (id) => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/paths/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
    ...
    const updateGraphNodeScore = async (conceptNodeId, isCorrect) => {
        ...
        if (nodeIndex !== -1) {
            ...
            const token = localStorage.getItem('auth_token');
            if (token) {
                await fetch(`${API_BASE}/api/paths/${currentPathId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
