...
export const llmService = {
    ...
    async getDoubtAnswer(question) {
        ...
        const response = await fetch(`${API_BASE}/api/ai/ask-rag`, {
            method: 'POST',
            headers: { 
    ...
    async *streamDoubtAnswer(question) {
        const token = localStorage.getItem('auth_token');
        const response = await fetch(`${API_BASE}/api/ai/stream-rag`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}` 
    ...
    async *streamGenericContent(prompt) {
        const response = await fetch(`${API_BASE}/api/ai/stream-generate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ provider: currentProvider, prompt })
        });
